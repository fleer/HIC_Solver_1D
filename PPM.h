...
class PPM : public NUMSCHEME
{
	...
	public:
	PPM(EOS * F, int N, double S, STATE iLEFT1,STATE iLEFT2, STATE iRIGHT1, STATE iRIGHT2, STATE PERT, double **VAR, double *POSITION, double *XSTEP): EOFSTATE(F), GRIDN(N), SYSTEMLENGTH(S)
	{
		//-----------
		...
		for(int I=ZERO; I<STOP; I++)
		{
			...
			if(X[I] > SYSTEMLENGTH/4. && X[I] < SYSTEMLENGTH/2.-0.5/SYSTEMLENGTH)
			{
				VAR[I][0]=iLEFT2.baryon;
				VAR[I][1]=iLEFT2.velx;
				VAR[I][2]=iLEFT2.pressure;
			}
			//input of pertubation
			else
			if(X[I] > (SYSTEMLENGTH/2.-0.5/SYSTEMLENGTH) && X[I] <  SYSTEMLENGTH/2.+0.5/SYSTEMLENGTH)
			{
				VAR[I][0]=PERT.baryon;
				VAR[I][1]=PERT.velx;
			...
			if(X[I] > SYSTEMLENGTH/2.+0.5/SYSTEMLENGTH && X[I] < 3*SYSTEMLENGTH/4.)
			{
