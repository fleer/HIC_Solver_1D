...
## Simulate
The program is able to solve simple Riemann Problems as the shocktube problem, but also more complicated problems.
...
Configuration files for the different types of problems are in `./Example_Config`, together with [gnuplot](http://www.gnuplot.info/) scripts for plotting *contourplots* and generating *gif-animations*.
...
### Shocktube-Problem
The Shocktube-Problem was introduced by Gary A. Sod in 1978 [5]. Here, the initial state is chosen in a way that the pressure - and by thus the temeprature - on the left side of the system is highert than on the right side. 

![Example](./images/shock_tube_problem.gif)

The configuration for the illustrated results can be found in `./Example_Configs/shock_tube_problem.ini`.
...
At the time the transversal pressure can no longer be ignored, a 3D hydrodynamics simulation has of course to be taken into consideration.

|![Landau](./images/landau_model.pdf)| ![LandauEvolve](./images/landau_model_evolve.pdf)|
|:--:|:--:|
|Initial State of the system in the Landau model|Evolution of the system in the Landau model|
...
Here, R is the radius of the nucleus, m its mass and sqrt(s) the energy of mass of the nucleus-nucles collision.
...
Additionally, a small pertubation can be implemented at $z=0$.
